...
void BeginLoadingDGO(const char* name, Ptr<u8> buffer1, Ptr<u8> buffer2, Ptr<u8> currentHeap) {
  ...
  RpcSync(DGO_RPC_CHANNEL[GameVersion::Jak3]);  // make sure old RPC is finished
  ...
  RpcCall(DGO_RPC_CHANNEL[GameVersion::Jak3], DGO_RPC_LOAD_FNO, true, mess, sizeof(RPC_Dgo_Cmd),
...
Ptr<u8> GetNextDGO(u32* lastObjectFlag) {
  *lastObjectFlag = 1;
  ...
  RpcSync(DGO_RPC_CHANNEL[GameVersion::Jak3]);
  Ptr<u8> buffer(0);
  if (sLastMsg) {
...
void ContinueLoadingDGO(Ptr<u8> b1, Ptr<u8> b2, Ptr<u8> heapPtr) {
  ...
  RpcCall(DGO_RPC_CHANNEL[GameVersion::Jak3], DGO_RPC_LOAD_NEXT_FNO, true, sendBuff,
          sizeof(jak3::RPC_Dgo_Cmd), sendBuff, sizeof(jak3::RPC_Dgo_Cmd));
